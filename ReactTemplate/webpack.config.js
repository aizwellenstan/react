...
const config = (isProd, isDevServer) => ({
  ...
  module: {
    rules: [
      ...
      {
        test: /\.(jpeg|png|gif|ttf|otf|eot|svg|woff(2)?)$/,
        use: [
          {
            loader: 'file-loader',
            options: {
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      'process.env': {
        ...
        APP_API: JSON.stringify(env.APP_API),
        APP_SERVERPROXY: JSON.stringify(env.APP_SERVERPROXY),
        APP_PROXY: JSON.stringify(env.APP_PROXY),
        APP_ALARMAPI: JSON.stringify(env.APP_ALARMAPI),
      },
    }),
